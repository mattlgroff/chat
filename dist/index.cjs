...
__export(src_exports, {
  Actions: () => Actions2,
  BaseFormatConverter: () => BaseFormatConverter,
  Button: () => Button2,
  Card: () => Card2,
  CardLink: () => CardLink2,
  CardText: () => CardText2,
  ...
  DEFAULT_EMOJI_MAP: () => DEFAULT_EMOJI_MAP,
  Divider: () => Divider2,
  ...
  StreamingMarkdownRenderer: () => StreamingMarkdownRenderer,
  THREAD_STATE_TTL_MS: () => THREAD_STATE_TTL_MS,
  Table: () => Table2,
  TextInput: () => TextInput2,
  ThreadImpl: () => ThreadImpl,
  blockquote: () => blockquote2,
  cardChildToFallbackText: () => cardChildToFallbackText2,
  codeBlock: () => codeBlock,
  convertEmojiPlaceholders: () => convertEmojiPlaceholders,
  createEmoji: () => createEmoji,
  defaultEmojiResolver: () => defaultEmojiResolver,
  deriveChannelId: () => deriveChannelId,
  emoji: () => emoji,
  emphasis: () => emphasis2,
  fromFullStream: () => fromFullStream,
  fromReactElement: () => fromReactElement2,
  fromReactModalElement: () => fromReactModalElement2,
  getEmoji: () => getEmoji,
  getNodeChildren: () => getNodeChildren,
  getNodeValue: () => getNodeValue,
  inlineCode: () => inlineCode2,
  isBlockquoteNode: () => isBlockquoteNode,
  isCardElement: () => isCardElement2,
  isCodeNode: () => isCodeNode,
  isDeleteNode: () => isDeleteNode,
  isEmphasisNode: () => isEmphasisNode,
  isInlineCodeNode: () => isInlineCodeNode,
  isJSX: () => isJSX2,
  isLinkNode: () => isLinkNode,
  isListItemNode: () => isListItemNode,
  isListNode: () => isListNode,
  isModalElement: () => isModalElement2,
  ...
  text: () => text5,
  toCardElement: () => toCardElement2,
  toModalElement: () => toModalElement2,
      ...
      if (events[index2][0] === "enter" && events[index2][1].type === "strikethroughSequenceTemporary" && events[index2][1]._close) {
        ...
        while (open--) {
          ...
          events[index2][1].end.offset - events[index2][1].start.offset === events[open][1].end.offset - events[open][1].start.offset) {
            ...
            splice(nextEvents, nextEvents.length, 0, [["exit", text6, context], ["enter", events[index2][1], context], ["exit", events[index2][1], context], ["exit", strikethrough2, context]]);
            splice(events, open - 1, index2 - open + 3, nextEvents);
...
function isStrongNode(node2) {
  return node2.type === "strong";
}
function isEmphasisNode(node2) {
  return node2.type === "emphasis";
}
function isDeleteNode(node2) {
  return node2.type === "delete";
}
function isInlineCodeNode(node2) {
  return node2.type === "inlineCode";
}
function isCodeNode(node2) {
  return node2.type === "code";
}
function isLinkNode(node2) {
  return node2.type === "link";
}
function isBlockquoteNode(node2) {
  return node2.type === "blockquote";
}
function isListNode(node2) {
  return node2.type === "list";
}
function isListItemNode(node2) {
  return node2.type === "listItem";
}
function isTableNode(node2) {
  return node2.type === "table";
}
function isTableRowNode(node2) {
  return node2.type === "tableRow";
}
function isTableCellNode(node2) {
  return node2.type === "tableCell";
}
function tableToAscii(node2) {
  const rows = [];
...
function walkAst(node2, visitor) {
  if ("children" in node2 && Array.isArray(node2.children)) {
...
function link2(url, children, title) {
...
var BaseFormatConverter = class {
  renderList(node2, depth, nodeConverter, unorderedBullet = "-") {
    ...
    for (const [i, item] of getNodeChildren(node2).entries()) {
      ...
      for (const child of getNodeChildren(item)) {
        ...
        const text6 = nodeConverter(child);
        if (!text6.trim()) {
          continue;
        }
        if (isFirstContent) {
          lines.push(`${indent2}${prefix} ${text6}`);
          isFirstContent = false;
        } else {
          lines.push(`${indent2}  ${text6}`);
        }
      }
    }
    return lines.join("\n");
  ...
  cardToFallbackText(card) {
    ...
    for (const child of card.children) {
      const text6 = this.cardChildToFallbackText(child);
      if (text6) {
        parts.push(text6);
      }
    }
...
function Text(content3, options = {}) {
  return {
    type: "text",
    content: content3,
    style: options.style
...
function fromReactElement(element) {
  ...
  switch (componentName) {
    ...
    case "Text": {
      const content3 = extractTextContent(props.children);
      return Text(content3, { style: props.style });
    }
    case "Image":
      return Image({
        url: props.url,
...
function cardToFallbackText(card) {
  ...
  for (const child of card.children) {
    const text6 = cardChildToFallbackText(child);
    if (text6) {
      parts.push(text6);
    }
  }
...
// packages/chat/src/chat-singleton.ts
var _singleton = null;
...
// packages/chat/src/jsx-runtime.ts
...
function resolveJSXElement(element) {
  ...
  if (type === Text) {
    ...
    const content3 = processedChildren.length > 0 ? processedChildren.map(String).join("") : String(textProps.children ?? "");
    return Text(content3, { style: textProps.style });
  }
  if (type === Section) {
...
var ChatError = class extends Error {
  code;
  cause;
  constructor(message, code3, cause) {
    super(message);
    this.name = "ChatError";
    this.code = code3;
    this.cause = cause;
...
function extractMessageContent(message) {
  if (typeof message === "string") {
    return {
      plainText: message,
      formatted: root2([paragraph2([text5(message)])]),
      attachments: []
    };
  }
  if ("raw" in message) {
    return {
      ...
      formatted: root2([paragraph2([text5(message.raw)])]),
  ...
  if ("card" in message) {
    ...
    return {
      ...
      formatted: root2([paragraph2([text5(fallbackText)])]),
      attachments: []
    };
  }
  if ("type" in message && message.type === "card") {
...
// packages/chat/src/from-full-stream.ts
...
// node_modules/.pnpm/remend@1.2.1/node_modules/remend/dist/index.js
...
var _ = (n, e, r) => e in n ? tn(n, e, { enumerable: true, configurable: true, writable: true, value: r }) : n[e] = r;
var k = (n, e) => {
...
var f = (n) => {
  ...
  return e >= 48 && e <= 57 || e >= 65 && e <= 90 || e >= 97 && e <= 122 || e === 95 ? true : W.test(n);
};
var d = (n, e) => {
  let r = false;
  for (let i = 0; i < e; i += 1) n[i] === "`" && n[i + 1] === "`" && n[i + 2] === "`" && (r = !r, i += 2);
  return r;
};
var G = (n, e) => {
  let r = 1;
...
var m = (n, e) => {
  for (let r = e - 1; r >= 0; r -= 1) {
    if (n[r] === ")") return false;
    if (n[r] === "(") return r > 0 && n[r - 1] === "]" ? fn(n, e) : false;
    if (n[r] === `
`) return false;
  }
  return false;
...
var p = (n, e, r) => {
  let i = 0;
  for (let o = e - 1; o >= 0; o -= 1) if (n[o] === `
`) {
    i = o + 1;
    break;
  }
  let s = n.length;
  for (let o = e; o < n.length; o += 1) if (n[o] === `
`) {
    s = o;
    break;
  }
  let t = n.substring(i, s), l = 0, c = false;
  for (let o of t) if (o === r) l += 1;
  else if (o !== " " && o !== "	") {
    c = true;
    break;
  }
...
var dn = /^(\s*(?:[-*+]|\d+[.)]) +)>(=?\s*[$]?\d)/gm;
var z = (n) => !n || typeof n != "string" || !n.includes(">") ? n : n.replace(dn, (e, r, i, s) => d(n, s) ? e : `${r}\\>${i}`);
var gn = (n, e, r, i) => r === "\\" || n.includes("$") && h(n, e) ? true : r !== "*" && i === "*" ? (e < n.length - 2 ? n[e + 2] : "") !== "*" : !!(r === "*" || r && i && f(r) && f(i) || (!r || r === " " || r === "	" || r === `
`) && (!i || i === " " || i === "	" || i === `
`));
var v = (n) => {
  ...
  for (let s = 0; s < i; s += 1) {
    ...
    if (r || n[s] !== "*") continue;
    let t = s > 0 ? n[s - 1] : "", l = s < i - 1 ? n[s + 1] : "";
    gn(n, s, t, l) || (e += 1);
  }
  return e;
};
var hn = (n, e, r, i) => !!(r === "\\" || n.includes("$") && h(n, e) || m(n, e) || F(n, e) || r === "_" || i === "_" || r && i && f(r) && f(i));
var mn = (n) => {
  let e = 0, r = false, i = n.length;
  for (let s = 0; s < i; s += 1) {
    if (n[s] === "`" && s + 2 < i && n[s + 1] === "`" && n[s + 2] === "`") {
...
var kn = (n, e, r) => {
  if (!e || u.test(e)) return true;
  let s = n.substring(0, r).lastIndexOf(`
`), t = s === -1 ? 0 : s + 1, l = n.substring(t, r);
...
var Y = (n) => {
  ...
  return d(n, i) || kn(n, r, i) ? n : C(n) % 2 === 1 ? r.endsWith("*") ? `${n}*` : `${n}**` : n;
};
var In = (n, e, r) => {
  if (!e || u.test(e)) return true;
  let s = n.substring(0, r).lastIndexOf(`
...
var j = (n) => {
  ...
  if (!e) {
    let t = n.match(K);
    if (t) {
      let l = n.lastIndexOf(t[1]);
      if (!d(n, l) && X(n) % 2 === 1) return `${n}_`;
    }
    return n;
  }
  let r = e[2], i = n.lastIndexOf(e[1]);
  return d(n, i) || In(n, r, i) ? n : X(n) % 2 === 1 ? `${n}__` : n;
};
var bn = (n) => {
  let e = false;
...
var Q = (n) => {
  if (!n.match(y)) return n;
  let r = bn(n);
  if (r === -1) return n;
  let i = n.substring(r + 1);
  return !i || u.test(i) ? n : v(n) % 2 === 1 ? `${n}*` : n;
};
var Z = (n) => {
  let e = false;
  for (let r = 0; r < n.length; r += 1) {
...
var Bn = (n) => {
  ...
  let e = n.slice(0, -2);
  if (C(e) % 2 !== 1) return null;
  let i = e.indexOf("**"), s = Z(e);
...
var q = (n) => {
  ...
  if (r === -1) return n;
  let i = n.substring(r + 1);
  if (!i || u.test(i)) return n;
  if (mn(n) % 2 === 1) {
    let t = Bn(n);
    return t !== null ? t : An(n);
  }
  return n;
};
var Cn = (n) => {
...
var Tn = (n, e, r) => !e || u.test(e) || d(n, r) ? true : p(n, r, "*");
var J = (n) => {
  if (D.test(n)) return n;
  let e = n.match(E);
  if (!e) return n;
  let r = e[2], i = n.lastIndexOf(e[1]);
  return Tn(n, r, i) ? n : pn(n) % 2 === 1 ? Cn(n) ? n : `${n}***` : n;
};
var g = (n, e) => {
  let r = false, i = false;
  for (let s = 0; s < e; s += 1) {
...
var V = (n) => {
  let e = 0;
  for (let r = 0; r < n.length; r += 1) n[r] === "`" && !Pn(n, r) && (e += 1);
  return e;
};
var On = /<[a-zA-Z/][^>]*$/;
var x = (n) => {
  let e = n.match(On);
  return !e || e.index === void 0 || g(n, e.index) ? n : n.substring(0, e.index).trimEnd();
};
var Sn = (n) => !n.match(H) || n.includes(`
...
var _n = (n, e) => e >= 2 && n.substring(e - 2, e + 1) === "```" || e >= 1 && n.substring(e - 1, e + 2) === "```" || e <= n.length - 3 && n.substring(e, e + 3) === "```";
var $n = (n) => {
  let e = 0, r = false;
  for (let i = 0; i < n.length - 1; i += 1) n[i] === "`" && !_n(n, i) && (r = !r), !r && n[i] === "$" && n[i + 1] === "$" && (e += 1, i += 1);
  return e;
};
var Ln = (n) => {
  let e = n.indexOf("$$");
...
var rn = (n) => $n(n) % 2 === 0 ? n : Ln(n);
var En = (n, e, r) => {
  if (n.substring(e + 2).includes(")")) return null;
  let s = G(n, e);
  if (s === -1 || g(n, s)) return null;
  let t = s > 0 && n[s - 1] === "!", l = t ? s - 1 : s, c = n.substring(0, l);
  if (t) return c;
  let o = n.substring(s + 1, e);
  return r === "text-only" ? `${c}${o}` : `${c}[${o}](streamdown:incomplete-link)`;
};
var en = (n, e) => {
  for (let r = 0; r < e; r++) if (n[r] === "[" && !g(n, r)) {
...
var yn = (n, e, r) => {
  ...
  if (!n.substring(e + 1).includes("]")) {
    ...
    if (r === "text-only") {
      let o = en(n, e);
      return n.substring(0, o) + n.substring(o + 1);
    }
...
var T = (n, e = "protocol") => {
  ...
  for (let i = n.length - 1; i >= 0; i -= 1) if (n[i] === "[" && !g(n, i)) {
    let s = yn(n, i, e);
    if (s !== null) return s;
  }
  return n;
...
`);
  ...
  if (Rn.test(s) && !r.match(Nn)) {
    let l = i.split(`
`).at(-1);
    if (l && l.trim().length > 0) return `${n}\u200B`;
  }
  if (Un.test(s) && !r.match(Wn)) {
    let l = i.split(`
`).at(-1);
    if (l && l.trim().length > 0) return `${n}\u200B`;
  }
...
var on = (n) => {
  ...
  if (e) {
    let s = e[2];
    if (!s || u.test(s)) return n;
    if (((r = n.match(A)) == null ? void 0 : r.length) % 2 === 1) return `${n}~~`;
  } else if (n.match(w) && ((i = n.match(A)) == null ? void 0 : i.length) % 2 === 1) return `${n}~`;
  return n;
};
var P = (n) => n !== false;
var a = { COMPARISON_OPERATORS: -10, HTML_TAGS: -5, SETEXT_HEADINGS: 0, LINKS: 10, BOLD_ITALIC: 20, BOLD: 30, ITALIC_DOUBLE_UNDERSCORE: 40, ITALIC_SINGLE_ASTERISK: 41, ITALIC_SINGLE_UNDERSCORE: 42, INLINE_CODE: 50, STRIKETHROUGH: 60, KATEX: 70, DEFAULT: 100 };
var Hn = [{ handler: { name: "comparisonOperators", handle: z, priority: a.COMPARISON_OPERATORS }, optionKey: "comparisonOperators" }, { handler: { name: "htmlTags", handle: x, priority: a.HTML_TAGS }, optionKey: "htmlTags" }, { handler: { name: "setextHeadings", handle: sn, priority: a.SETEXT_HEADINGS }, optionKey: "setextHeadings" }, { handler: { name: "links", handle: T, priority: a.LINKS }, optionKey: "links", earlyReturn: (n) => n.endsWith("](streamdown:incomplete-link)") }, { handler: { name: "boldItalic", handle: J, priority: a.BOLD_ITALIC }, optionKey: "boldItalic" }, { handler: { name: "bold", handle: Y, priority: a.BOLD }, optionKey: "bold" }, { handler: { name: "italicDoubleUnderscore", handle: j, priority: a.ITALIC_DOUBLE_UNDERSCORE }, optionKey: "italic" }, { handler: { name: "italicSingleAsterisk", handle: Q, priority: a.ITALIC_SINGLE_ASTERISK }, optionKey: "italic" }, { handler: { name: "italicSingleUnderscore", handle: q, priority: a.ITALIC_SINGLE_UNDERSCORE }, optionKey: "italic" }, { handler: { name: "inlineCode", handle: nn, priority: a.INLINE_CODE }, optionKey: "inlineCode" }, { handler: { name: "strikethrough", handle: on, priority: a.STRIKETHROUGH }, optionKey: "strikethrough" }, { handler: { name: "katex", handle: rn, priority: a.KATEX }, optionKey: "katex" }];
var Dn = (n) => {
  var r;
  let e = (r = n == null ? void 0 : n.linkMode) != null ? r : "protocol";
  return Hn.filter(({ handler: i, optionKey: s }) => i.name === "links" ? P(n == null ? void 0 : n.links) || P(n == null ? void 0 : n.images) : P(n == null ? void 0 : n[s])).map(({ handler: i, earlyReturn: s }) => i.name === "links" ? { handler: I(k({}, i), { handle: (t) => T(t, e) }), earlyReturn: e === "protocol" ? s : void 0 } : { handler: i, earlyReturn: s });
};
var Kn = (n, e) => {
  var l;
  if (!n || typeof n != "string") return n;
  let r = n.endsWith(" ") && !n.endsWith("  ") ? n.slice(0, -1) : n, i = Dn(e), s = ((l = e == null ? void 0 : e.handlers) != null ? l : []).map((c) => {
    var o;
    return { handler: I(k({}, c), { priority: (o = c.priority) != null ? o : a.DEFAULT }), earlyReturn: void 0 };
  }), t = [...i, ...s].sort((c, o) => {
    var O, S;
    return ((O = c.handler.priority) != null ? O : 0) - ((S = o.handler.priority) != null ? S : 0);
  });
  for (let { handler: c, earlyReturn: o } of t) if (r = c.handle(r), o != null && o(r)) return r;
  return r;
};
var pr = Kn;
...
var StreamingMarkdownRenderer = class {
  ...
  render() {
    ...
    this.dirty = false;
    if (this.finished) {
      this.cachedRender = pr(this.accumulated);
      return this.cachedRender;
    }
    ...
    this.cachedRender = pr(committable);
    return this.cachedRender;
  }
  /**
  ...
  getCommittableText() {
    ...
    let text6 = this.accumulated;
    if (text6.length > 0 && !text6.endsWith("\n")) {
      const lastNewline = text6.lastIndexOf("\n");
      const withoutIncompleteLine = lastNewline >= 0 ? text6.slice(0, lastNewline + 1) : "";
      if (isInsideCodeFence(withoutIncompleteLine)) {
        return wrapTablesForAppend(text6);
      }
      text6 = withoutIncompleteLine;
    }
    if (isInsideCodeFence(text6)) {
      return wrapTablesForAppend(text6);
    }
    const committed = getCommittablePrefix(text6);
...
function isClean(text6) {
  return pr(text6).length <= text6.length;
}
function findCleanPrefix(text6) {
  if (text6.length === 0 || isClean(text6)) {
    return text6;
  }
  for (let i = text6.length - 1; i >= 0; i--) {
    if (INLINE_MARKER_CHARS.has(text6[i])) {
      while (i > 0 && text6[i - 1] === text6[i]) {
        i--;
      }
      const candidate = text6.slice(0, i);
...
function isInsideCodeFence(text6) {
  let inside = false;
...
function getCommittablePrefix(text6) {
  const endsWithNewline = text6.endsWith("\n");
...
function wrapTablesForAppend(text6, closeFences = false) {
  const hadTrailingNewline = text6.endsWith("\n");
  const lines = text6.split("\n");
...
var ThreadImpl = class _ThreadImpl {
  ...
  async fallbackStream(textStream, options) {
    ...
    const doEditAndReschedule = async () => {
      ...
      const content3 = renderer.render();
      if (content3 !== lastEditContent) {
        try {
          await this.adapter.editMessage(threadIdForEdits, msg.id, {
            markdown: content3
          });
          lastEditContent = content3;
        } catch {
        }
      }
      if (!stopped) {
...
function convertEmojiPlaceholders(text6, platform, resolver2 = defaultEmojiResolver) {
  return text6.replace(EMOJI_PLACEHOLDER_REGEX, (_2, emojiName) => {
    switch (platform) {
      case "slack":
        return `:${resolver2.toSlack(emojiName)}:`;
      case "gchat":
        return resolver2.toGChat(emojiName);
      case "teams":
        return resolver2.toGChat(emojiName);
      case "discord":
        return resolver2.toDiscord(emojiName);
      case "github":
        return resolver2.toGChat(emojiName);
      case "linear":
        return resolver2.toGChat(emojiName);
